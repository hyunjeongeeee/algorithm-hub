...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        */
        ...
        if(max >= H) {  // 제일큰 거인의 키가 센티보다 클경우에만 작동

        // 뿅망치 잔여횟수가 남아있고 + 제일큰 거인의 키가 센티보다 클경우에만 작동
            while(T-- > 0)  {

                if(max!=1){ // 거인의 키가 1이 아닐경우 (1이면 아무 영향도 받지 않는다)
                    count++;
                    max = giants.poll()/ 2;  // 제일 큰값을 빼서 반으로 망치질
                    giants.add(max);    // 다시 pq에 넣어줌
                }

                if (giants.peek() < H) {
                    break;
                } else if(giants.peek() >= H) {
                    max = giants.peek();
                }
            }
        }

        // T를 소진하고도 (센치 >= 거인) false인 경우
        if(max < H) {
            bw.write("YES");
            bw.write("\n"+ count);
        } else {
